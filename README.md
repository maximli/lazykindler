...
#### 关于备份
在设计之初，后台数据库就选择了sqlite，目的就是为了方便保存。平台所有的信息包括书籍都存储在sqlite中，针对每一个本书，把解析出的书籍封面也保存到了单独的表中，即使没有本平台，使用简单的脚本就可以把电子书和封面提取出来。目前sqlite文件的位置在 `backend/src/database/lazykindler.db`
