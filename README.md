...
#### 关于备份
平台的数据库采用了 sqlite3，目的是为了方便备份。启动初次平台时，会自动创建并初始化该数据库文件。数据库位置时 `backend/lazykindler.db`，该文件包含了平台的所有数据信息。另外往平台导入的书籍都被复制到了 `backend/data` 目录下，也很方便备份。需要注意的是，平台为了方便操作，在 `backend/data` 下的书籍名后面都被添加了 `md5`。
